...
namespace Foreman.Controls {
    ...
    public class MouseHoverDetector : IDisposable {
        ...
        public MouseHoverDetector(int hoverTimeMilliseconds = 200, int reshowTimeMilliseconds = 200, int hoverGraceDistance = 15) {
            items = [];
            timer = new System.Windows.Forms.Timer { Enabled = false, Interval = 50 };
        ...
        public void Add(Control control, MouseEventHandler hoverStartEventHandler, EventHandler hoverEndEventHandler) {
            if (items.TryGetValue(control, out var info)) {
        ...
        private void control_MouseMove(object? sender, MouseEventArgs e) {
            if (sender is not Control ctrl)
                return;
            lastMouseMoveControl = ctrl;
            Info info = items[ctrl];
            ...
            if (info.IsHovering) {
                if (Math.Abs(info.HoverStartPoint.X - e.Location.X) + Math.Abs(info.HoverStartPoint.Y - e.Location.Y) > HoverGraceDistance) {
                    ...
                    info.HoverEndHandler?.Invoke(sender, EventArgs.Empty);
                }
        ...
        private void timer_Tick(object? sender, EventArgs e) {
            if (lastMouseMoveControl is null)
                return;
            ...
            if (!info.IsHovering && now - info.LastMoveTime > HoverTime) {
                ...
                info.HoverStartHandler?.Invoke(lastMouseMoveControl, new MouseEventArgs(MouseButtons.None, 0, info.HoverStartPoint.X, info.HoverStartPoint.Y, 0));
            }
        }
